...
//       User.findById(viewerId).select("friends followers").lean(),
//     ]);
...
//     const viewerFollowers = (viewer?.followers || []).map(String);
//     // viewerFollowing = who the viewer follows = viewer.friends[]
//     const viewerFollowing = (viewer?.friends || []).map(String);
//     // "Following" means viewerId is in target.followers[]
//     const targetFollowers = (target.followers || []).map(String);
//     const isFollowing = targetFollowers.includes(viewerId);
//     const isFriend = viewerFriends.includes(userId);
...
//     const target = await User.findById(userId).select("followers name").lean();
//     if (!target)
//       return res
//         .status(404)
//         .json({ success: false, message: "User not found" });

//     const alreadyFollowing = (target.followers || [])
//       .map(String)
//       .includes(viewerId);

//     if (alreadyFollowing) {
//       // Unfollow:
//       //   • Remove viewerId from target.followers  (target loses a follower)
//       //   • Remove targetId  from viewer.friends   (viewer loses a following)
//       await Promise.all([
//         User.findByIdAndUpdate(userId, { $pull: { followers: viewerId } }),
//         User.findByIdAndUpdate(viewerId, { $pull: { friends: userId } }),
//       ]);
//     } else {
//       // Follow:
//       //   • Add viewerId to target.followers  (target gains a follower)
//       //   • Add targetId  to viewer.friends   (viewer gains a following — shown as friendCount on their profile)
//       await Promise.all([
//         User.findByIdAndUpdate(userId, { $addToSet: { followers: viewerId } }),
//         User.findByIdAndUpdate(viewerId, { $addToSet: { friends: userId } }),
//       ]);
...
//           actor: viewerId,
//           type: "follow",
//           message: `${viewer?.name || "Someone"} started following you`,
//         });
//       }
//     }

//     // Return live counts so the frontend can update without a full reload
//     const [updatedTarget, updatedViewer] = await Promise.all([
//       User.findById(userId).select("followers friends").lean(),
...
//         : `Now following ${target.name}`,
//       // Live counts the UI can render immediately
//       targetFollowers: (updatedTarget?.followers || []).length,
//       targetFriends: (updatedTarget?.friends || []).length,
//       viewerFollowing: (updatedViewer?.friends || []).length,
//     });
//   } catch (err) {
