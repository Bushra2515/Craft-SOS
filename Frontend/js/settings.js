...
function markDirty() {
  if (_loading) return; // ignore programmatic changes during load
  dirty = true;
...
async function loadSettings() {
  ...
  try {
    ...
    updateBioChars();
    syncPreview();
    // ── Reset dirty state — populating fields from the API is NOT a user edit ──

    // Reset dirty — loading from API is not a user edit.
    // 600ms delay covers browser autofill which fires oninput asynchronously.
    dirty = false;
...
async function saveProfile() {
  ...
  const payload = {
    ...
    // Tell the backend to clear the avatar if the user clicked Remove
    ...(_avatarRemoved ? { removeAvatar: true } : {}),
  };
...
function removeAvatar() {
  const avCircle = document.querySelector(".av-circle");
  if (avCircle) {
    avCircle.style.backgroundImage = "";
    avCircle.style.backgroundSize = "";
    avCircle.style.backgroundPosition = "";
    // Restore initials
    const fn = g("firstName"),
      ln = g("lastName");
    ...
    avCircle.prepend(document.createTextNode(initials));
  }
  const prevAv = document.getElementById("prev-av");
  if (prevAv) {
    ...
    const fn = g("firstName"),
      ln = g("lastName");
    prevAv.textContent = ((fn[0] || "") + (ln[0] || "")).toUpperCase() || "?";
  }
  const stored = JSON.parse(localStorage.getItem("user") || "{}");
  ...
  _avatarRemoved = true;
  markDirty();
  showToast("Avatar removed — save to confirm.");
}
// Must be on window so the HTML onclick attribute can reach it
