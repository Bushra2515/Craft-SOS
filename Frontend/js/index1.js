...
async function loadFeed() {
  ...
  try {
    ...
    //         <a href="pages/post-detail.html?id=${post._id}" class="post-link">
    //          ${esc(post.title)}
    //         </a>
    //       </h2>
    //       <h2 class="card-heading">${esc(post.title)}</h2>
    ...
    posts.forEach((post) => {
      ...
      article.innerHTML = `
        ...
        <div class="card-body">
          <h2 class="card-heading">
            <a href="pages/post-detail.html?id=${post._id || post.id}" class="post-link">
              ${esc(post.title)}
            </a>
          </h2>
          <p class="card-para">${esc(post.body)}</p>
        ...
        <div class="card-footer">
          ...
          <div class="card-acts">
            ...
            <button class="icon-btn save-btn" title="Save"
                    onclick="savePost('${post._id || post.id}', this)">
              ${post.isSaved ? "🔖" : "🤍"}
            </button>
          </div>
        </div>
