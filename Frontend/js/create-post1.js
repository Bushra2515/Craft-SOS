...
async function publishPost() {
  ...
  // innerText = plain text only, used for the minimum-length guard
  const bodyText = (
    document.getElementById("editor-area").innerText || ""
  ).trim();

  // innerHTML = full rich HTML — preserves bold, italic, headings, lists,
  // blockquotes, code blocks, etc. This is what gets stored in the database.
  // Using innerText here (the original bug) stripped ALL formatting before
  // the fetch call, so nothing was ever saved to the server.
  const bodyHTML = document.getElementById("editor-area").innerHTML || "";
