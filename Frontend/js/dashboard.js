...
async function loadProgress() {
  try {
    ...
    renderBadges(badges && badges.length ? badges : []);
    renderPtsBreakdown();
...
function renderFeed(posts) {
  ...
  el.innerHTML = posts
    .map(
      (p) => `
    <div class="feed-item">
      <div class="fi-av" style="${p.author?.avatar ? `background-image:url(${p.author.avatar});background-size:cover;` : "background:#a8c4d8"}">${p.author?.avatar ? "" : (p.author?.name || "?")[0].toUpperCase()}</div>
      <div class="fi-body">
        <div class="fi-top">
        ...
        <div class="fi-foot">
          ...
          <span class="fi-read" onclick="window.location.href='post-detail.html?id=${p.id}'">Read →</span>
        </div>
      </div>
    </div>`,
    )
...
function renderSosPulse(posts) {
  ...
  el.innerHTML = posts
    .map(
      (s) => `
    <div class="sos-live" onclick="window.location.href='post-detail.html?id=${s.id}'">
...
function renderOpenPostsWidget(posts) {
  ...
  el.innerHTML = posts
    ...
    <div class="open-post">
      <div class="op-top">
        <div class="op-title" onclick="window.location.href='post-detail.html?id=${p.id}'">${esc(p.title)}</div>
        <button class="op-edit-btn" onclick="window.location.href='edit-post.html?id=${p.id}'"
          title="Edit post" style="background:none;border:none;cursor:pointer;color:var(--text-light);padding:2px 6px;border-radius:6px;font-size:.78rem;flex-shrink:0;white-space:nowrap">
          ✏️ Edit
        </button>
      </div>
      <div class="op-meta">
...
function renderNotifications(notifs) {
  ...
  el.innerHTML = notifs
    .map(
      (n) => `
    <div class="notif-item${n.isRead ? "" : " unread"}" onclick="switchSection('notifications', null)">
      <div class="notif-dot ${n.type}">
        ${{ reply: "💬", helped: "✅", badge: "🏆", sos: "🆘", follow: "👤", milestone: "🎉", friend_request: "🤝", friend_accepted: "✅" }[n.type] || "🔔"}
      </div>
...
function renderPeople(users) {
  ...
  el.innerHTML = users
    ...
    <div class="people-item">
      <div class="pp-av" style="${u.avatar ? `background-image:url(${u.avatar});background-size:cover;` : "background:#a8b8d0"}">${u.avatar ? "" : (u.name || "?")[0].toUpperCase()}</div>
      <div class="pp-info">
...
function renderBadges(badges) {
  ...
  // Home section shelf — same badges, with empty state
  const homeShelf = document.getElementById("home-badge-shelf");
  const homeEmpty = document.getElementById("home-badge-empty");
  if (homeShelf) {
...
══════════════════════════════════════════════════ */
let _myPosts = [];
let _myPostsFilter = "all"; // "all" | "sos" | "tut" | "resolved"

/* ── Filter buttons in My Posts section ── */
function filterMyPosts(type, btn) {
  _myPostsFilter = type;
  document
    .querySelectorAll(".my-posts-filter-btn")
    .forEach((b) => b.classList.remove("on"));
  btn.classList.add("on");
  renderMyPostsFull();
}

/* ── Delete a post directly from the dashboard ──
   DELETE /api/posts/:id  (same endpoint as edit-post.js)        */
async function deleteDashboardPost(postId, btn) {
  if (!confirm("Delete this post permanently? This cannot be undone.")) return;

  const card = btn.closest(".open-post");
  if (card) {
  ...
  try {
    ...
    // Remove from cached array + re-render
...
function renderMyPostsFull() {
  ...
  // Apply filter first so empty-state messages are filter-aware
  const filtered = _myPosts.filter(
    (p) =>
      _myPostsFilter === "all" ||
      p.type === _myPostsFilter ||
      (_myPostsFilter === "resolved" && p.status === "resolved"),
  );

  if (!_myPosts.length) {
    el.innerHTML = `<div class="loading-placeholder">No posts yet.</div>
      <button class="btn ghost" style="margin-top:4px" onclick="window.location.href='create-post.html'">
  ...
  if (!filtered.length) {
    ...
    el.innerHTML = `<div class="loading-placeholder">No ${LABELS[_myPostsFilter] || "posts"} yet.</div>`;
    return;
  }
  ...
  el.innerHTML =
    filtered
      .map(
        (p) => `
    <div class="open-post" style="margin-bottom:12px" data-post-id="${p.id}">
      <div class="op-top">
        <div class="op-title" onclick="window.location.href='post-detail.html?id=${p.id}'">${esc(p.title)}</div>
        <div class="op-actions" style="display:flex;gap:6px;flex-shrink:0">
          <button onclick="window.location.href='edit-post.html?id=${p.id}'"
            style="background:none;border:1px solid var(--accent);color:var(--accent);cursor:pointer;padding:3px 10px;border-radius:6px;font-size:.78rem;white-space:nowrap">
            ✏️ Edit
          </button>
          <button onclick="deleteDashboardPost('${p.id}', this)"
            style="background:none;border:1px solid #e0827a;color:#c0392b;cursor:pointer;padding:3px 10px;border-radius:6px;font-size:.78rem;white-space:nowrap">
            🗑 Delete
          </button>
        </div>
      </div>
...
document.addEventListener("DOMContentLoaded", async () => {
  // Sidebar — common.js provides initSidebar
  initSidebar();
  ...
  loadProgress(); // eagerly fetch badges so home-badge-shelf is populated
});
