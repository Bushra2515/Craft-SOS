...
(function () {
  ...
  /* ─── Styles — mirrors site's section-card visual language ── */
  function injectStyles() {
    if (document.getElementById("cw-styles")) return;
    const style = document.createElement("style");
    style.id = "cw-styles";
    style.textContent = `
      /* ── Card shell — matches .section-card ── */
      .cw-widget {
        background: var(--card, #fff);
        border: 1px solid var(--border, rgba(122,143,82,.18));
        border-radius: 16px;
        overflow: hidden;
        margin-bottom: 18px;
      }

      /* ── Header — matches .section-title ── */
      .cw-widget-head {
        display: flex;
        align-items: center;
        gap: 8px;
        padding: 14px 18px 12px;
        border-bottom: 1px solid var(--border, rgba(122,143,82,.15));
      }
      .cw-widget-title {
        font-family: 'Lora', Georgia, serif;
        font-weight: 700;
      ...
      /* icon inside title — matches .section-title-text svg */
      .cw-widget-title svg {
        width: 15px; height: 15px;
        stroke: var(--accent, #7a8f52);
        fill: none;
        stroke-width: 2;
      ...
      .cw-ch-item {
        ...
        gap: 10px;
        padding: 10px 0;
        border-bottom: 1px solid var(--border, rgba(122,143,82,.12));
      }
      .cw-ch-item:last-child { border-bottom: none; padding-bottom: 0; }
      .cw-ch-ico {
        width: 36px; height: 36px;
      ...
      .cw-ch-dot { width: 3px; height: 3px; border-radius: 50%; background: var(--border, #ccc); }
      .cw-ch-deadline { color: #c0432a; font-weight: 600; }

      /* Join — matches .btn-outline pill style */
      .cw-join-btn {
        font-size: 0.72rem; font-weight: 700;
        padding: 5px 12px;
        border-radius: 20px;
        border: 1.5px solid var(--accent, #7a8f52);
        background: transparent;
        color: var(--accent, #7a8f52);
      ...
      .cw-join-btn:hover { background: rgba(122,143,82,.1); }
      .cw-join-btn.joined { background: var(--accent, #7a8f52); color: #fff; border-color: var(--accent, #7a8f52); }
      .cw-join-btn:disabled { opacity: .5; cursor: default; }
      ...
      .cw-ann-banner:last-child { margin-bottom: 0; }
      .cw-ann-ico { font-size: 1rem; flex-shrink: 0; line-height: 1.5; }
      .cw-ann-content { flex: 1; min-width: 0; }
      .cw-ann-title { font-weight: 700; font-size: 0.82rem; margin-bottom: 2px; }
      .cw-ann-body { font-size: 0.76rem; line-height: 1.45; color: var(--text-mid, #6b6560); overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; }
      .cw-ann-time { font-size: 0.68rem; color: var(--text-light, #9a9690); margin-top: 3px; }
      .cw-ann-close { position: absolute; top: 8px; right: 10px; background: none; border: none; font-size: 0.9rem; cursor: pointer; color: var(--text-light, #9a9690); padding: 0; font-family: inherit; }
      .cw-ann-close:hover { color: var(--text, #2c2822); }
      .cw-ann-announcement { background: rgba(58,110,168,.07); border-color: rgba(58,110,168,.2); }
      .cw-ann-announcement .cw-ann-title { color: #3a6ea8; }
      .cw-ann-challenge { background: rgba(122,143,82,.08); border-color: rgba(122,143,82,.2); }
      .cw-ann-challenge .cw-ann-title { color: var(--accent, #7a8f52); }
      .cw-ann-safety_notice { background: rgba(192,67,42,.07); border-color: rgba(192,67,42,.2); }
      .cw-ann-safety_notice .cw-ann-title { color: #c0432a; }
      .cw-ann-spotlight, .cw-ann-feature_update { background: rgba(114,88,176,.07); border-color: rgba(114,88,176,.2); }
      .cw-ann-spotlight .cw-ann-title, .cw-ann-feature_update .cw-ann-title { color: #7258b0; }
      .cw-ann-notification { background: rgba(176,120,32,.08); border-color: rgba(176,120,32,.2); }
      .cw-ann-notification .cw-ann-title { color: #b07820; }

      /* ── Badges ── */
      .cw-badge-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; }
      .cw-badge-tile {
        background: rgba(122,143,82,.07);
        border: 1px solid var(--border, rgba(122,143,82,.15));
        border-radius: 12px; padding: 12px 6px; text-align: center;
        transition: transform .18s, box-shadow .18s; cursor: default;
      }
      .cw-badge-tile:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(122,143,82,.15); }
      .cw-badge-em { font-size: 1.5rem; line-height: 1; margin-bottom: 5px; }
      .cw-badge-name { font-size: 0.69rem; font-weight: 700; color: var(--text, #2c2822); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      .cw-badge-cnt { font-size: 0.65rem; color: var(--text-light, #9a9690); margin-top: 2px; }
      .cw-badge-latest {
        margin-top: 12px; padding-top: 10px;
        border-top: 1px solid var(--border, rgba(122,143,82,.15));
        font-size: 0.75rem; color: var(--text-mid, #6b6560);
        display: flex; align-items: center; gap: 8px;
      }
      .cw-badge-latest-em { font-size: 1.1rem; }
      .cw-badge-latest strong { color: var(--text, #2c2822); }

      /* ── Shimmer — earthy green tint matching site's loading shimmers ── */
      .cw-shimmer {
        background: linear-gradient(90deg,
          rgba(122,143,82,.08) 25%,
          rgba(122,143,82,.16) 50%,
          rgba(122,143,82,.08) 75%);
        background-size: 200% 100%;
        animation: cwShimmer 1.4s infinite;
        border-radius: 8px;
      }
    ...
    try {
      ...
      if (!chals.length) {
        root.innerHTML = `
          <div class="cw-widget">
            ...
            <div class="cw-widget-body"><div class="cw-empty">No active challenges right now.</div></div>
          </div>`;
        return;
      }
          ...
          <div class="cw-widget-body">
            ${chals
              ...
              .map((c) => {
                ...
                return `
              <div class="cw-ch-item">
                <div class="cw-ch-ico">${esc(c.icon || "🎯")}</div>
                <div class="cw-ch-info">
                  <div class="cw-ch-title" title="${esc(c.title)}">${esc(c.title)}</div>
                  <div class="cw-ch-meta">
                    <span>👥 ${c.participantCount}</span>
                ...
                <button
                  class="cw-join-btn ${c.joined ? "joined" : ""}"
                  id="cw-join-${c._id}"
                  onclick="cwJoinChallenge('${c._id}', this)"
      ...
      root.innerHTML = `
        <div class="cw-widget">
          ...
          <div class="cw-widget-body"><div class="cw-empty">Couldn't load challenges.</div></div>
        </div>`;
      console.warn("[community-widgets/challenges]", err.message);
    }
  }
  ...
  window.cwJoinChallenge = async function (cid, btn) {
    ...
    try {
      ...
      if (meta) {
        const msg = document.createElement("span");
        msg.textContent = data.joined ? "🎉 You're in!" : "Left challenge";
        msg.style.cssText = `color:${data.joined ? "var(--accent,#7a8f52)" : "#9a9690"};font-weight:600;`;
        meta.appendChild(msg);
        setTimeout(() => msg.remove(), 2500);
      }
    } catch (err) {
      ...
      if (!badges.length) {
        root.innerHTML = `
          <div class="cw-widget">
            ...
            <div class="cw-widget-body"><div class="cw-empty">No badges yet.</div></div>
          </div>`;
        return;
      }
            ...
            ${
              ...
              <div class="cw-badge-latest">
                ...
                <div>
                  <strong>@${esc(latest.userHandle)}</strong> just earned
                  <strong>${esc(latest.badge)}</strong>
                  <span style="color:var(--text-light,#9a9690);font-size:0.68rem;display:block;">${timeAgo(latest.awardedAt)}</span>
                </div>
              </div>`
                : ""
            }
